...
import { Headphones, LogIn, Menu, UserRound, X } from "lucide-react";
    ...
    >
        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <Button asChild variant="outline" className="hidden sm:inline-flex">
              <Link to="/dashboard"><UserRound className="size-4" /> Account</Link>
            </Button>
          ) : (
            <Button type="button" variant="outline" className="hidden sm:inline-flex" onClick={() => openAuth("login")}>
      ...
      {open ? (
        <div className="mx-5 mt-3 overflow-hidden rounded-3xl border border-border bg-card p-3 shadow-card lg:hidden">
          ...
          >
            <Headphones className="size-4 text-primary" /> Open Live Support
          </button>

          {user ? (
            <Button asChild variant="outline" size="lg" className="mt-3 w-full"><Link to="/dashboard"><UserRound className="size-4" /> My Account</Link></Button>
          ) : (
            <Button type="button" variant="outline" size="lg" className="mt-3 w-full" onClick={() => openAuth("login")}><LogIn className="size-4" /> Sign in</Button>
          )}
          <Button asChild variant="premium" size="lg" className="mt-2 w-full">
            <Link to="/contact">Start Project</Link>
          </Button>
        </div>
      ) : null}
